...
export class Defence {
    ...
    public run() {
       ...
       //logic of the last row
        let isPastLastDefesiceline = Math.abs(ballPosition.y) > 3;
        amun.log(isPastLastDefesiceline);
        let intersectionPoint = calculateVectorOfIntersection(-3, ballPosition, friendlyGoal);
        if(isPastLastDefesiceline && (Math.abs(ballPosition.x) > 2)){
            intersectionPoint = new Vector(0,-3);
        }
        for (let i = 0; i < back.length; i++) {
            moveTo(i, intersectionPoint.add(new Vector(i/6,0)), back);
        }
        ...
        for (let i = 0; i < middle.length; i++) {
            let p = new Vector((-fieldHeight / 2) + (fieldHeight / (middle.length + 1)) * (i+1), -1.5);
        ...
        for (let i = 0; i < front.length; i++) {
           if(i != closestFront){
               moveTo(i, front[closestFront].pos.add(new Vector(i/2, i/2)), front);
           }
        }

        //logic of the keeper
        amun.log("The keeper is: " + keeper + " distance to ball: " + keeper.pos.distanceTo(ballPosition));
        if (keeper.pos.distanceTo(ballPosition) < 1 && keeper.pos.distanceTo(friendlyGoal) < 1) {
